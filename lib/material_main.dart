...
`
              ...
              if(snapshot.connectionState == ConnectionState.waiting){
                ...
                if(snapshot.hasError){
                  ...
                  return ListView.separated(
                    itemCount: (snapshot.data as List<Todo>).length, //형 변환 후 length 사용
  ...
  Future<void> _createTodo() async {
    Todo todo = await Navigator.of(context).pushNamed('/create') as Todo; //Todo로 다운캐스팅
  ...
  Future<void> _updateTodo(Todo todo) async {
    ...
    Todo res = await showDialog(
      context: context,
      builder: (context) {
        return AlertDialog( //상태는 바뀌지만 화면이 갱신되지 않음 (AlertDialog는 Stateless 위젯)
          title: Text("${todo.id}: ${todo.title}"),
          content: StatefulBuilder( //StatefulBuilder를 이용하여 화면 갱신
